...
export const getTennisResult = createAsyncThunk(
  ...
  async (payload: any) => {
    var response = await getRequest(
      `${SportBaseUrl}/tennis/result/${payload?.tourId}`
    )
    if (response?.status === 200 || response?.status === 201) {
      return response?.data
    }
...
export const getTennisGames = createAsyncThunk(
  ...
  async (payload: any) => {
    var response = await getRequest(
      `${SportBaseUrl}/tennis/games/${payload?.period}`
    )
    if (response?.status === 200 || response?.status === 201) {
      return response?.data
    }
...
export const tennisFixtureStatusState = (state: RootState) =>
  state.tennis.loading
