...
export const getFxRate = createAsyncThunk(
  ...
  async (payload: any) => {
    const buildUrl = (payload) => {
      ...
      return `${SportSportBaseUrl}/misc/fx-rates?${queryString}`;
    };
...
export const getTransferRate = createAsyncThunk(
  ...
  async (payload: any) => {
    const buildUrl = (payload) => {
      ...
      return `${SportSportBaseUrl}/misc/transfer-rates?${queryString}`;
    };
...
export const getBankList = createAsyncThunk(
  ...
  async () => {
    const buildUrl = () => {
      ...
      return `${SportSportBaseUrl}/misc/banks`;
    };
...
export const getBankBranch = createAsyncThunk(
  ...
  async (payload: any) => {
    const buildUrl = (payload) => {
      ...
      return `${SportSportBaseUrl}/misc/banks/${payload?.id}/branches`;
    };
    var response = await getRequest(buildUrl(payload));
    if (response?.status === 200 || response?.status === 201) {
      return response?.data;
...
export const verifyBank = createAsyncThunk(
  ...
  async (payload: any, { rejectWithValue }) => {
    ...
    try {
      const response = await postRequest(`${SportSportBaseUrl}/misc/resolve-bank-account`, payload);
      if (response?.status === 200 || response?.status === 201) {
        return response;
      }
    } catch (e: any) {
